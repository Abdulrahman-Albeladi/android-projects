...
TEXT_SUFFIXES = {
    ".py", ".pyi", ".toml", ".cfg", ".ini", ".txt", ".md",
    ".yml", ".yaml", ".json", ".sh", ".bash", ".xml",
    ".js", ".jsx", ".ts", ".tsx", ".java", ".kt", ".kts",
    ".c", ".h", ".cpp", ".hpp", ".rs", ".ml", ".mli",
    ".gradle", ".properties",
}

SKIP_PARTS = {
    ".git", ".venv", "venv", "node_modules", "build", "dist",
    ".gradle", "target", "_build", "__pycache__", ".pytest_cache",
...
for path in files():
    ...
    if path.stat().st_size > 95_000_000:
        failures.append(f"File exceeds 95 MB: {relative}")
    ...
    if suffix == ".ipynb":
        try:
            ...
            failures.append(f"Malformed notebook {relative}: {exc}")
            continue
        ...
        for index, cell in enumerate(notebook.cells):
            if cell.cell_type == "code":
                if cell.get("outputs"):
                    failures.append(
                        f"Notebook output remains: {relative}, cell {index}"
                    )
                if cell.get("execution_count") is not None:
                    failures.append(
                        f"Execution count remains: {relative}, cell {index}"
                    )
        ...
        try:
            ...
            failures.append(f"Invalid XML {relative}: {exc}")

    if suffix in TEXT_SUFFIXES and path.stat().st_size <= 5_000_000:
        text = path.read_text(encoding="utf-8", errors="ignore")
        for kind, pattern in SECRET_PATTERNS.items():
            if pattern.search(text):
                failures.append(f"{kind} pattern in {relative}")

if failures:
    print("\n".join(failures))
    raise SystemExit(1)

print("Portfolio structural, security, syntax, and notebook checks passed.")
